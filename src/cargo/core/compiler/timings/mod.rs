...
use crate::ops::cargo_report::timings::LogProvider;
...
impl<'gctx> Timings<'gctx> {
    ...
    pub fn unit_finished(
        ...
        let Some(unit_time) = self.active.remove(&id) else {
            return;
        };
        let elapsed = self.start.elapsed().as_secs_f64();
        let unblocked = unblocked.iter().map(|u| self.unit_to_index[u]).collect();
        let log_message = LogMessage::UnitFinished {
                index: self.unit_to_index[&unit_time.unit],
                elapsed,
                unblocked,
            };
        ...
        if let Some(logger) = build_runner.bcx.logger {
            ...
            self.unit_times.push(log_message);
        }
    }
    ...
    pub fn finished(
        ...
        if self.report_html {
            // The `cargo build --timings` feature will be superceded by `cargo report timings` subcommand
            // So relying on its logics for now (bit messy)
            let render_ctx = prepare_context(LogProvider::Vec(self.unit_times.clone()), &RunId::new(&build_runner.bcx.ws.root()))?;

            let timestamp = render_ctx.start_str.replace(&['-', ':'][..], "");
            ...
            write_html(render_ctx, &mut out_file)?;
            let link = self.gctx.shell().err_file_hyperlink(&filename);
            let msg = format!("report saved to {link}{}{link:#}", filename.display());
            self.gctx.shell()
